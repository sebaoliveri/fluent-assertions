...
object OptionalExp {
  def maybeStringConstant[T,R](option: Option[R]): OptionalExp[T, R] = maybeStringVariable(_ => option)
...
case class OptionalBoolExp[T,R](constantExp: OptionalExp[T,R], expressionProvider: R => BooleanExp[T,Bool]) extends BooleanExp[T,Bool] {
  override def evaluate(context: T): Bool = constantExp.evaluate(context).map(optionValue => expressionProvider(optionValue).evaluate(context)).getOrElse(TrueExp)
}
