...
import expression.{Bool, IsDefinedExp, IterableExp, LogicalOperatorsExp, NullExp, ObjectExp, OptionalBoolExp, OptionalExp}
...
object OptionalIterableExpAssertionBuilder {
  ...
  def apply[T,R](optionalExp: OptionalExp[T,Iterable[R]], expression: LogicalOperatorsExp[T,Bool]): OptionalIterableExpAssertionBuilder[T,R] =
    new OptionalIterableExpAssertionBuilder(optionalExp, expression, _ and _)
}

case class OptionalIterableExpAssertionBuilder[T,R](optionExp: OptionalExp[T,Iterable[R]], expression: LogicalOperatorsExp[T,Bool], operator: (LogicalOperatorsExp[T,Bool], LogicalOperatorsExp[T,Bool]) => LogicalOperatorsExp[T,Bool])
  ...
  def forAll(predicate: R => LogicalOperatorsExp[Unit,Bool]): OptionalIterableExpAssertionBuilder[T,R] =
    newWith(iterableConstant(_).forAll(indexBoundedTo(predicate)))

  def existAny(predicate: R => LogicalOperatorsExp[Unit,Bool]): OptionalIterableExpAssertionBuilder[T,R] =
  ...
  private def newWith(newExpression: Iterable[R] => LogicalOperatorsExp[T,Bool]): OptionalIterableExpAssertionBuilder[T,R] =
